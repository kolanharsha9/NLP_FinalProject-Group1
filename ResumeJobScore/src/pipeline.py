...
from ResumeJobScore.src.utils.file_utils import load_json
from ResumeJobScore.src.preprocess.resume_processor import preprocess_resume
from ResumeJobScore.src.preprocess.job_desc_processor import preprocess_job_desc
...
def preprocess_pipeline(resume_dir, job_desc_dir, output_dir, batch_size=1000):
    ...
    for i in tqdm(range(0, len(resume_files), batch_size), desc="Resumes", unit="batch"):
        ...
        try:
            ...
            with torch.no_grad():
                embeddings = model(**inputs).last_hidden_state.cpu()
