...
def main_page():
    ...
    if resume_file and job_description_text.strip():
        ...
        if parsed_resume and parsed_job_description:
            ...
            st.warning(
                "Failed to parse the resume or job description. Ensure the file format is correct and the text is readable.")
        ...
        if resume_text.strip():
            ...
            for i, job in enumerate(recommended_jobs, 1):
                with st.expander(f"{i}. {job['title']}"):
                    ...
                    st.write(
                        f"**Experience Required:** {job.get('experience_required', {}).get('min_years', 'N/A')} years")
...
def ResumeGFScore():
    ...
    # Page heading
    st.markdown("<h1 style='text-align: center; margin-top: 2em;'>Grammar and Format Check</h1>",
