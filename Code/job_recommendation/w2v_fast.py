...
class SemanticJobRecommender:
    def __init__(self, 
                 ...
                 min_count=1):
        """
        Initialize the semantic job recommender with optimized embeddings.
        
        :param jobs_data_path: Path to jobs JSON file
        :param embeddings_path: Path to save/load Word2Vec embeddings
        :param cache_path: Path to save/load precomputed job vectors
        """
        # Set up paths
        ...
        if cache_path is None:
            cache_path = os.path.join(script_dir, 'job_vectors_cache.joblib')
        ...
        # Load or create embeddings and job vectors
        self.model = self._load_or_create_embeddings()
        self.job_vectors = self._load_or_create_job_vectors()
    
    def _advanced_text_preprocessing(self, text):
        """
        Advanced text preprocessing technique with optimization.
    ...
    def _load_or_create_embeddings(self):
        """
        Load existing embeddings or create new ones with optimization.
        ...
        model = Word2Vec(
            ...
            sg=1,  # Use skip-gram model (generally faster for large datasets)
    ...
    def _load_or_create_job_vectors(self):
        """
        Load or create precomputed job vectors to speed up recommendations.
        
        :return: List of job vectors
        """
        # Try to load cached job vectors
        if os.path.exists(self.cache_path):
            try:
                print("Loading cached job vectors...")
                return joblib.load(self.cache_path)
            except Exception as e:
                print(f"Error loading job vectors cache: {e}. Generating new vectors.")
        ...
        # Cache job vectors
        print(f"Caching job vectors to {self.cache_path}")
        joblib.dump(job_vectors, self.cache_path)
    ...
    def recommend_jobs(self, resume_text, top_n=5):
        """
        Recommend jobs using Word2Vec semantic matching with optimization.
        ...
        # Compute cosine similarities more efficiently
    ...
    def read_pdf_resume(pdf_path):
        """
        Read text content from a PDF resume with improved extraction.
